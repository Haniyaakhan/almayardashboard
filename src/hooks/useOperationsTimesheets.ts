...
export function useLaborTimesheets(month?: number, year?: number) {
...
/** Fetches only approved labor timesheets, filtered by month/year. */
...
export function useInvoiceTimesheets(month?: number, year?: number) {
  const [timesheets, setTimesheets] = useState<Timesheet[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  const fetch = useCallback(async () => {
    ...
    let q = supabase
      ...
      .in('sheet_type', ['labor', 'tunnel_employee'])
      .order('year', { ascending: false })
      .order('month', { ascending: false })
      .order('created_at', { ascending: false });
