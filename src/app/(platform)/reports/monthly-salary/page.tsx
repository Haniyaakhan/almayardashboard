...
export default function SalaryPerMonthPage() {
  ...
  const reportData = {
    headers: ['Employee Name', 'Designation', 'Basic Salary', 'OT Hours', 'Advances', 'Foreman Commission', 'Net Salary', 'Status'],
    rows: report.salaryDetails.map((s, idx) => ({
      cells: [
        s.full_name,
        s.designation || 'Unspecified',
        s.basic_salary.toFixed(2),
        s.overtime_hours.toFixed(2),
        s.advances_amount.toFixed(2),
        s.foreman_commission.toFixed(2),
