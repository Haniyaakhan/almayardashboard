...
function TunnelVehicleTimesheetPageInner() {
  ...
  // Ensure default project name for this tunnel vehicle timesheet
  useEffect(() => {
    const tsId = searchParams.get('ts');
    const vehicleId = searchParams.get('vehicle');
    if (!tsId && !vehicleId && (!timesheet.projectName || !timesheet.projectName.trim())) {
      timesheet.setProjectName(DEFAULT_PROJECT_NAME);
    }
  }, []); // run once on mount
  ...
  return (
    <div style={{ background: '#f5f5f5', minHeight: '100%' }}>
      ...
      <div className="p-4" style={{ paddingBottom: 24 }}>
        ...
        >
          <TimesheetHeader title="Time Sheet" timesheetType="tunnel_vehicle" />
          <InfoTable
...
export default function TunnelVehicleTimesheetPage() {
  return (
    <Suspense>
      <TunnelVehicleTimesheetPageInner />
    </Suspense>
  );
}
