...
export function LaborRegistryPage() {
  ...
  async function syncAllBanks() {
    const toSync = laborers.filter((l) => l.bank_name && l.bank_name.trim());
    if (!toSync.length) { toast.warning('No laborers have a bank name set'); return; }
    ...
    for (const lab of toSync) {
      const canonical = OMAN_BANK_LIST.find((b) => resolveSwift(lab.bank_name) === b.swift);
      if (!canonical) continue; // unknown bank — skip
      const needsNameFix = canonical.name !== lab.bank_name;
      ...
      // Always sync entry bank_name to canonical
      if (draftIds.length > 0) {
        const { error } = await supabase
          .from('salary_sheet_entries')
          .update({ bank_name: canonical.name })
    ...
    toast.success(
      `Bank sync complete — ${labUpdated} laborer name${labUpdated !== 1 ? 's' : ''} normalised, ${entryUpdated} draft entr${entryUpdated !== 1 ? 'ies' : 'y'} updated`
    );
    ...
    <div>
      ...
      <div style={{
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <RefreshCw size={14} /> {syncingBanks ? 'SYNCING…' : 'SYNC BANKS'}
          </button>
...
type BulkResult = { processed: number; updated: number; skipped: number };

function normalizeHeader(h: string): string {
  return h.toLowerCase().replace(/[\s_\-]/g, '');
}

// Only bank fields are updatable via the upload template
const LABOUR_COLUMN_MAP: Record<string, keyof Laborer> = {
...
function rowsToPreview(
  ...
  laborers: Laborer[],
): { preview: BulkPreviewItem[]; skipped: BulkSkippedItem[] } {
  ...
  rows.forEach((row, idx) => {
    ...
    if (!laborer) { skipped.push({ rowIndex: idx + 2, labourId, reason: `Labour ID "${labourId}" not found` }); return; }

    const changes: Partial<Laborer> = {};
    const fieldsChanged: string[] = [];

    Object.entries(row).forEach(([header, rawValue]) => {
      const field = LABOUR_COLUMN_MAP[normalizeHeader(header)];
      if (!field || field === 'id_number') return;
      if (!rawValue) return;
      if (String((laborer as any)[field] ?? '') !== rawValue) {
        (changes as any)[field] = rawValue;
...
function BulkUpdateModal({
  ...
  async function handleFile(file: File) {
    try {
      ...
      setParsedRows(rows);
      const { preview: p, skipped: s } = rowsToPreview(rows, laborers);
      setPreview(p);
      setSkipped(s);
      setStep('preview');
    } catch (err: any) {
  ...
  async function applyUpdates() {
    ...
    // Fetch all draft sheet IDs once so we can sync entries below
    const supabase = createClient();
    ...
    for (const item of preview) {
      ...
      if (err) {
        ...
        updated++;
        // Sync bank fields into draft salary sheet entries (approved sheets stay frozen)
        if (draftSheetIds.length > 0) {
          ...
          if (Object.keys(entryPatch).length > 0) {
            // Match by labor_code (id_number) — more reliable than laborer_id UUID
    ...
    if (updated > 0 && draftSheetIds.length > 0) {
      toast.success(`Updated ${updated} laborer${updated === 1 ? '' : 's'} and synced salary sheet entries`);
    }
    setResult({ processed: parsedRows.length, updated, skipped: skippedCount });
    setApplying(false);
    setStep('done');
  }
  ...
  async function downloadTemplate() {
    ...
    // Hidden sheet for bank dropdown validation
    ...
    // Add data validation dropdown for Bank_Name (rows 2-200)
    for (let r = 2; r <= 200; r++) {
    ...
    // Example row
    const exampleBank = 'Bank Muscat';
    ws.addRow({
      Labour_ID: '12345',
      Labour_Name: 'Example Worker',
      Bank_Name: exampleBank,
      Bank_SWIFT_Code: resolveSwift(exampleBank),
      Bank_Account_Number: 'OM1234567890',
    });
  ...
  const maxWidth = step === 'preview' ? 980 : 640;
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: '3px 0 0' }}>
              {step === 'upload' && 'Upload CSV or Excel file to update multiple labour records at once'}
              {step === 'preview' && `${preview.length} record${preview.length !== 1 ? 's' : ''} to update · ${skipped.length} skipped`}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: '#eff6ff', borderRadius: 10, border: '1px solid #bfdbfe' }}>
              ...
              <div style={{ flex: 1, fontSize: 12, color: '#1e40af' }}>
                <strong>Bank Details Template</strong> — columns: Labour ID, Labour Name, Bank Name, Bank Account Number.
                Fill in the rows and upload to update bank details only.
              </div>
              <button onClick={downloadTemplate} style={{ fontSize: 12, fontWeight: 600, color: '#3b82f6', background: 'white', border: '1px solid #bfdbfe', borderRadius: 7, padding: '5px 14px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                Download .xlsx
              </button>
            </div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.75, padding: '10px 14px', background: 'var(--input-bg)', borderRadius: 10, border: '1px solid var(--border)' }}>
              ...
              <ul style={{ margin: '4px 0 0 16px', padding: 0 }}>
                ...
                <li>Rows with missing or invalid Labour ID are skipped automatically</li>
          ...
          <div>
            {preview.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ maxHeight: 320, overflowY: 'auto', overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 10 }}>
            ...
            {skipped.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ maxHeight: 170, overflowY: 'auto', border: '1px solid #fecaca', borderRadius: 10 }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            ...
            {preview.length === 0 && skipped.length > 0 && (
              <div style={{ padding: '16px 18px', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 10, fontSize: 13, color: '#92400e', marginBottom: 16 }}>
                No records to update. All rows were skipped. Please check your file and try again.
              </div>
            )}

            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button onClick={() => { setStep('upload'); setParsedRows([]); setPreview([]); setSkipped([]); }} style={{ padding: '9px 18px', borderRadius: 9, border: '1px solid var(--border)', background: 'var(--bg-card)', fontSize: 13, cursor: 'pointer', color: 'var(--text-light)', fontWeight: 600 }}>
                Back
              </button>
              <button
                onClick={applyUpdates}
                disabled={applying || !preview.length}
                style={{ ...addBtnStyle, background: '#4f46e5', opacity: (applying || !preview.length) ? 0.6 : 1, cursor: applying ? 'wait' : (!preview.length ? 'not-allowed' : 'pointer') }}
              >
                {applying ? 'Applying…' : `Apply ${preview.length} Update${preview.length !== 1 ? 's' : ''}`}
              </button>
            </div>
          </div>
        )}
